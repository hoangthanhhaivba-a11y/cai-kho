...
# --- Khởi tạo Client Gemini (Chỉ chạy một lần, dùng caching) ---
@st.cache_resource
def get_gemini_client():
    """Lấy và cache đối tượng Gemini Client."""
...
if gemini_client is None:
    st.stop() # Dừng ứng dụng nếu không có client
...
def process_financial_data(df):
    ...
    for col in numeric_cols:
        # Thay thế 'errors='coerce'' để chuyển các giá trị không phải số thành NaN, sau đó fillna(0)
    ...
    tong_tai_san_row = df[df['Chỉ tiêu'].str.contains('TỔNG CỘNG TÀI SẢN|TỔNG TÀI SẢN', case=False, na=False)]
...
# --- HÀM MỚI: Xử lý Chat hỏi đáp với Gemini (ỔN ĐỊNH) ---
...
def run_gemini_chat_interface(df_processed):
    ...
    # Gán dữ liệu phân tích vào một biến state để tránh tạo lại string lớn này liên tục
    if "system_instruction" not in st.session_state:
        data_markdown = df_processed.to_markdown(index=False)
    ...
    if prompt := st.chat_input("Hỏi Gemini về dữ liệu đã tải lên..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Đang xử lý, Gemini đang phân tích dữ liệu..."):
                ...
                # Gọi API với toàn bộ lịch sử trò chuyện và System Prompt 
...
if uploaded_file is not None:
    try:
        ...
        # Đảm bảo có đúng 3 cột, nếu không sẽ gây lỗi index
        if df_raw.shape[1] < 3:
             raise ValueError("File Excel cần ít nhất 3 cột: Chỉ tiêu, Năm trước, Năm sau.")
             
        df_raw.columns = ['Chỉ tiêu', 'Năm trước', 'Năm sau']
        
        # Reset trạng thái chat và prompt khi file mới được tải lên
        ...
        if df_processed is not None:
            ...
            thanh_toan_hien_hanh_N = 0.0
            thanh_toan_hien_hanh_N_1 = 0.0
            
            try:
                # Lọc giá trị cần thiết
            ...
            # Chuẩn bị dữ liệu để gửi cho AI (Dữ liệu này sẽ được làm tròn trước khi gửi)
    ...
    # Reset trạng thái khi không có file
    if "chat_messages" in st.session_state:
        del st.session_state["chat_messages"]
